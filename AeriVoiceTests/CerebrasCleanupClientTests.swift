import Foundation
import XCTest

@testable import AeriVoice

final class CerebrasCleanupClientTests: XCTestCase {
  override func setUp() {
    super.setUp()
    CerebrasURLProtocolStub.handler = nil
  }

  func testCleanupUsesDirectCerebrasStructuredOutputAndNoneReasoning() async throws {
    CerebrasURLProtocolStub.handler = { request in
      XCTAssertEqual(request.url?.absoluteString, "https://api.cerebras.ai/v1/chat/completions")
      XCTAssertEqual(request.httpMethod, "POST")
      XCTAssertEqual(request.timeoutInterval, 10)
      XCTAssertEqual(request.value(forHTTPHeaderField: "Authorization"), "Bearer test-key")
      let body = try XCTUnwrap(request.cerebrasBodyData)
      let json = try XCTUnwrap(JSONSerialization.jsonObject(with: body) as? [String: Any])
      XCTAssertEqual(json["model"] as? String, "qwen-3.8-27b")
      XCTAssertEqual(json["reasoning_effort"] as? String, "none")
      XCTAssertEqual(json["max_completion_tokens"] as? Int, 256)
      let responseFormat = try XCTUnwrap(json["response_format"] as? [String: Any])
      XCTAssertEqual(responseFormat["type"] as? String, "json_schema")
      let jsonSchema = try XCTUnwrap(responseFormat["json_schema"] as? [String: Any])
      XCTAssertEqual(jsonSchema["strict"] as? Bool, true)
      let response =
        #"{"model":"qwen-3.8-27b","service_tier":"shared","usage":{"prompt_tokens":22,"completion_tokens":5,"total_tokens":27},"choices":[{"message":{"content":"{\"text\":\"Hello, world.\"}"}}]}"#
        .data(using: .utf8)!
      return (
        HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!,
        response
      )
    }

    let result = try await CerebrasCleanupClient(session: makeSession()).clean(
      "hello world", mode: .faithful,
      configuration: CleanupConfiguration(model: .qwen38_27BCerebras, reasoningEffort: .none),
      apiKey: "test-key")

    XCTAssertEqual(result.text, "Hello, world.")
    XCTAssertEqual(result.metrics.actualModel, "qwen-3.8-27b")
    XCTAssertEqual(result.metrics.selectedProvider, "Cerebras")
    XCTAssertEqual(result.metrics.selectedProviderModel, "qwen-3.8-27b")
    XCTAssertEqual(result.metrics.routingStrategy, "direct")
    XCTAssertEqual(result.metrics.routingAttempt, 1)
    XCTAssertEqual(result.metrics.serviceTier, "shared")
    XCTAssertEqual(result.metrics.totalTokens, 27)
    XCTAssertEqual(result.metrics.httpStatus, 200)
  }

  func testConfiguredReasoningEffortsAreEncodedExactly() async throws {
    let efforts: [CleanupReasoningEffort] = [.low, .medium, .high]
    for effort in efforts {
      CerebrasURLProtocolStub.handler = { request in
        let body = try XCTUnwrap(request.cerebrasBodyData)
        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: body) as? [String: Any])
        XCTAssertEqual(json["reasoning_effort"] as? String, effort.rawValue)
        let response = #"{"choices":[{"message":{"content":"{\"text\":\"Cleaned.\"}"}}]}"#
          .data(using: .utf8)!
        return (
          HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!,
          response
        )
      }

      let result = try await CerebrasCleanupClient(session: makeSession()).clean(
        "raw", mode: .faithful,
        configuration: CleanupConfiguration(model: .qwen38_27BCerebras, reasoningEffort: effort),
        apiKey: "test-key")
      XCTAssertEqual(result.text, "Cleaned.")
    }
  }

  func testThinkingTagsAreStrippedIfPresentInContent() async throws {
    CerebrasURLProtocolStub.handler = { request in
      let response =
        #"{"choices":[{"message":{"content":"<think>Internal thought process</think>{\"text\":\"Polished output.\"}"}}]}"#
        .data(using: .utf8)!
      return (
        HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!,
        response
      )
    }

    let result = try await CerebrasCleanupClient(session: makeSession()).clean(
      "raw", mode: .polished,
      configuration: CleanupConfiguration(model: .qwen38_27BCerebras, reasoningEffort: .low),
      apiKey: "test-key")
    XCTAssertEqual(result.text, "Polished output.")
  }

  func testValidationChecksCatalogAndExercisesSelectedModel() async throws {
    var requestCount = 0
    CerebrasURLProtocolStub.handler = { request in
      requestCount += 1
      let response: Data
      if request.url?.path == "/v1/models" {
        XCTAssertEqual(request.httpMethod, "GET")
        response = #"{"data":[{"id":"qwen-3.8-27b"}]}"#.data(using: .utf8)!
      } else {
        XCTAssertEqual(request.url?.path, "/v1/chat/completions")
        let body = try XCTUnwrap(request.cerebrasBodyData)
        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: body) as? [String: Any])
        XCTAssertEqual(json["model"] as? String, "qwen-3.8-27b")
        XCTAssertEqual(json["reasoning_effort"] as? String, "none")
        XCTAssertEqual(json["max_completion_tokens"] as? Int, 256)
        response = #"{"choices":[{"message":{"content":"{\"text\":\"Test.\"}"}}]}"#
          .data(using: .utf8)!
      }
      return (
        HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!,
        response
      )
    }

    try await CerebrasCleanupClient(session: makeSession()).validate(apiKey: "test-key")

    XCTAssertEqual(requestCount, 2)
  }

  func testValidationRejectsKeyWhenModelIsMissingFromCatalog() async {
    CerebrasURLProtocolStub.handler = { request in
      XCTAssertEqual(request.url?.path, "/v1/models")
      return (
        HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!,
        #"{"data":[{"id":"other-model"}]}"#.data(using: .utf8)!
      )
    }

    do {
      try await CerebrasCleanupClient(session: makeSession()).validate(apiKey: "test-key")
      XCTFail("Expected model missing error")
    } catch {
      XCTAssertTrue(error.localizedDescription.contains("Qwen 3.8 27B is unavailable"))
    }
  }

  func testProviderErrorIncludesCerebrasMetrics() async {
    CerebrasURLProtocolStub.handler = { request in
      let response = #"{"error":{"message":"Rate limit exceeded"}}"#.data(using: .utf8)!
      return (
        HTTPURLResponse(url: request.url!, statusCode: 429, httpVersion: nil, headerFields: nil)!,
        response
      )
    }

    do {
      _ = try await CerebrasCleanupClient(session: makeSession()).clean(
        "raw", mode: .faithful,
        configuration: CleanupConfiguration(model: .qwen38_27BCerebras, reasoningEffort: .none),
        apiKey: "test-key")
      XCTFail("Expected 429 to fail")
    } catch {
      XCTAssertEqual(
        error.localizedDescription, "Cerebras is temporarily rate limited. Wait a moment and try again."
      )
      XCTAssertEqual((error as? ProviderHTTPError)?.statusCode, 429)
      XCTAssertEqual((error as? ProviderHTTPError)?.cleanupMetrics?.selectedProvider, "Cerebras")
      XCTAssertEqual(
        (error as? ProviderHTTPError)?.cleanupMetrics?.selectedProviderModel,
        "qwen-3.8-27b")
    }
  }

  func testTokenBudgetGrowsWithInputAndStaysWithinTotalLimit() throws {
    XCTAssertEqual(try CerebrasTokenBudget.maxCompletionTokens(for: "Test."), 256)
    XCTAssertEqual(
      try CerebrasTokenBudget.maxCompletionTokens(for: String(repeating: "a", count: 8_000)), 2_500)
    XCTAssertEqual(
      try CerebrasTokenBudget.maxCompletionTokens(for: String(repeating: "é", count: 1_000)), 1_250)

    let text = String(repeating: "a", count: 12_000)
    let completionTokens = try CerebrasTokenBudget.maxCompletionTokens(for: text)
    XCTAssertLessThanOrEqual(
      CerebrasTokenBudget.estimatedTokens(for: text) + CerebrasTokenBudget.requestOverheadTokens
        + completionTokens,
      CerebrasTokenBudget.totalTokenLimit)
  }

  func testTokenBudgetRejectsDictationThatCannotFitInputAndOutput() {
    XCTAssertThrowsError(
      try CerebrasTokenBudget.maxCompletionTokens(for: String(repeating: "a", count: 70_000))
    ) { error in
      XCTAssertEqual(
        error.localizedDescription,
        "This dictation is too long for Cerebras’s current limit. Use OpenRouter or try a shorter dictation."
      )
    }
  }

  func testOversizedRequestUsesConciseActionableError() async {
    CerebrasURLProtocolStub.handler = { request in
      let response = #"{"message":"Payload too large"}"#.data(using: .utf8)!
      return (
        HTTPURLResponse(url: request.url!, statusCode: 413, httpVersion: nil, headerFields: nil)!,
        response
      )
    }

    do {
      _ = try await CerebrasCleanupClient(session: makeSession()).clean(
        "raw", mode: .faithful,
        configuration: CleanupConfiguration(model: .qwen38_27BCerebras, reasoningEffort: .none),
        apiKey: "test-key")
      XCTFail("Expected 413 to fail")
    } catch {
      XCTAssertEqual(
        error.localizedDescription,
        "This request is larger than your Cerebras plan allows. Try a shorter dictation or raise your Cerebras limits."
      )
    }
  }

  func testMalformedSuccessIncludesKnownCerebrasAttemptMetrics() async {
    CerebrasURLProtocolStub.handler = { request in
      let response = #"{"model":"qwen-3.8-27b","choices":[]}"#.data(using: .utf8)!
      return (
        HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!,
        response
      )
    }

    do {
      _ = try await CerebrasCleanupClient(session: makeSession()).clean(
        "raw", mode: .faithful,
        configuration: CleanupConfiguration(model: .qwen38_27BCerebras, reasoningEffort: .none),
        apiKey: "test-key")
      XCTFail("Expected malformed cleanup to fail")
    } catch {
      let providerError = error as? ProviderHTTPError
      XCTAssertEqual(providerError?.statusCode, 200)
      XCTAssertEqual(providerError?.cleanupMetrics?.actualModel, "qwen-3.8-27b")
      XCTAssertEqual(providerError?.cleanupMetrics?.selectedProvider, "Cerebras")
      XCTAssertEqual(providerError?.cleanupMetrics?.routingStrategy, "direct")
      XCTAssertEqual(providerError?.cleanupMetrics?.routingAttempt, 1)
    }
  }

  func testCleanupRouterUsesCerebrasForCerebrasModel() async throws {
    let session = makeSession()
    CerebrasURLProtocolStub.handler = { request in
      XCTAssertEqual(request.url?.host, "api.cerebras.ai")
      let response = #"{"choices":[{"message":{"content":"{\"text\":\"Cleaned by Cerebras.\"}"}}]}"#
        .data(using: .utf8)!
      return (
        HTTPURLResponse(
          url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!,
        response
      )
    }
    let router = CleanupClientRouter(
      openRouter: OpenRouterCleanupClient(session: session),
      groq: GroqCleanupClient(session: session),
      cerebras: CerebrasCleanupClient(session: session))

    let result = try await router.clean(
      "raw", mode: .faithful,
      configuration: CleanupConfiguration(model: .qwen38_27BCerebras, reasoningEffort: .none),
      apiKey: "test-key")

    XCTAssertEqual(result.text, "Cleaned by Cerebras.")
  }

  private func makeSession() -> URLSession {
    let configuration = URLSessionConfiguration.ephemeral
    configuration.protocolClasses = [CerebrasURLProtocolStub.self]
    return URLSession(configuration: configuration)
  }
}

extension URLRequest {
  fileprivate var cerebrasBodyData: Data? {
    if let httpBody { return httpBody }
    guard let stream = httpBodyStream else { return nil }
    stream.open()
    defer { stream.close() }
    var data = Data()
    var buffer = [UInt8](repeating: 0, count: 4096)
    while stream.hasBytesAvailable {
      let count = stream.read(&buffer, maxLength: buffer.count)
      guard count > 0 else { break }
      data.append(buffer, count: count)
    }
    return data
  }
}

private final class CerebrasURLProtocolStub: URLProtocol, @unchecked Sendable {
  nonisolated(unsafe) static var handler: ((URLRequest) throws -> (HTTPURLResponse, Data))?

  override class func canInit(with request: URLRequest) -> Bool { true }
  override class func canonicalRequest(for request: URLRequest) -> URLRequest { request }
  override func startLoading() {
    do {
      let result = try Self.handler!(request)
      client?.urlProtocol(self, didReceive: result.0, cacheStoragePolicy: .notAllowed)
      client?.urlProtocol(self, didLoad: result.1)
      client?.urlProtocolDidFinishLoading(self)
    } catch {
      client?.urlProtocol(self, didFailWithError: error)
    }
  }
  override func stopLoading() {}
}
