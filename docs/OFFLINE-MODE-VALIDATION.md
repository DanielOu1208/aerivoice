...
### Compact onboarding and settings

Onboarding and Dictation now share one model selector with Cloud and On this Mac groups. Initial onboarding prominently offers local models; local setup has one status, an Apple-only language row, an info button for longer explanations, and an accuracy footer. Onboarding hides cleanup while offline. Providers uses the same compact setup while preserving removal, retry, and partial-download recovery.

The full app suite passed 411 tests with 3 live-cloud skips; Release build `2026091404` passed. Code review confirmed that selecting a cloud model exits Offline mode through the guarded network transition, while changing local engines offline retains the saved cloud provider. A compiler crash on a directly passed actor-isolated setter was resolved by using an explicit binding closure.

Isolated native onboarding QA passed at 780 × 720: one model/language/readiness display, no cleanup offline, Apple → Nemotron → Offline off restoring Soniox, direct offline Apple → Soniox exposing cloud key setup, and info-popover content. No keys were entered, models downloaded or removed, or permissions granted. The original user preview stayed open during this validation.

Isolated settings QA also passed: Dictation's combined selector and cloud switching, compact Apple management with one Ready status and accuracy footer, and installed Nemotron's enabled removal control. Removal was not performed, and missing/download/retry states were covered by code/tests rather than this installed-assets UI pass. Escape from the Nemotron info popover dismissed the management sheet as well. The QA app was quit; the original preview remained open.

With user authorization, signed build `2026091404` replaced `/Applications/AeriVoice.app` and launched with `--show-onboarding`. Deep strict signature verification, matching the prior app's designated signing requirement, candidate/installed executable hash equality, bundle/build checks, and the running executable path passed. Saved `onboardingComplete` remained `1`; the first-step request is transient. Backup: `/Applications/.AeriVoice-before-compact-onboarding-2026091404.app`. Signing required the desktop Keychain approval flow; no permissions, credentials, or model assets were reset.
